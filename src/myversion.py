...
class Plugin(CliPlugin):
    ...
    def _fetch_state(self, state):
        ...
        software_version_path = build_path(
            '/system/app-management/application[name="idb_server"]'
        )
        control_path = build_path('/platform/control[slot="*"]')

        try:
            self._hostname_data = state.server_data_store.get_data(
                hostname_path, recursive=True
            )
        except ServerError:
            self._hostname_data = None

        try:
            self._chassis_data = state.server_data_store.get_data(
                chassis_path, recursive=True
            )
        except ServerError:
            self._chassis_data = None

        try:
            self._software_version = state.server_data_store.get_data(
                software_version_path, recursive=True
            )
        except ServerError:
            self._software_version = None

        try:
            self._control_data = state.server_data_store.get_data(
                control_path, recursive=True
            )
        except ServerError:
    ...
    def _populate_data(self, state, arguments):
        ...
        if self._hostname_data:
            data.hostname = (
                self._hostname_data.system.get().name.get().host_name or data.hostname
            )
        ...
        if self._chassis_data:
            data.chassis_type = (
                self._chassis_data.platform.get().chassis.get().type
                or data.chassis_type
            )
            data.part_number = (
                self._chassis_data.platform.get().chassis.get().part_number
                or data.part_number
            )
            data.serial_number = (
                self._chassis_data.platform.get().chassis.get().serial_number
                or data.serial_number
            )
            data.system_hw_mac_address = (
                self._chassis_data.platform.get().chassis.get().hw_mac_address
                or data.system_hw_mac_address
            )
            data.last_booted = (
                self._chassis_data.platform.get().chassis.get().last_booted
                or data.last_booted
            )
        ...
        if self._software_version:
            if (
                self._software_version.system.get()
                .app_management.get()
                .application.exists("idb_server")
            ):
                sw_version = (
                    self._software_version.system.get()
                    .app_management.get()
                    .application.get("idb_server")
                    .version
                )
        ...
        if self._control_data:
            for control_slot in ["A", "B"]:
                if self._control_data.platform.get().control.exists(control_slot):
                    ctrl_data = self._control_data.platform.get().control.get(
                        control_slot
                    )
